...
UPDATES_CHANNEL = os.environ.get('UPDATES_CHANNEL', 'Wick')
...
async def genStr(_, msg: Message):
    if msg.chat.id in Credentials.BANNED_USERS:
        await bot.send_message(
            chat_id=msg.chat.id,
            text="You are Banned. Contact My [Support Group](url)",
    ...
    if update_channel:
        try:
            ...
            if user.status == "kicked":
               await bot.send_message(
                   chat_id=msg.chat.id,
                   text="Sorry Sir, You are Banned. Contact My [Support Group](url).",
                   parse_mode="markdown",
            ...
            await bot.send_message(
                chat_id=msg.chat.id,
                text="**Something Went Wrong. Contact My [Support Group](url).**",
                parse_mode="markdown",
                disable_web_page_preview=True
            )
            return
    ...
    try:
        ...
        await client.send_message("me", f"#PYROGRAM #STRING_SESSION \n\n```{session_string}``` \n\nBy [String Session Generator](http://t.me/WickStringSessionRobot) 🤖\nMade with ❤️ By @AsmSafone! 👑")
        await client.disconnect()
...
"""
    reply_markup = InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton('DEVELOPER', url='Wick'),
            ]
        ]
    )
